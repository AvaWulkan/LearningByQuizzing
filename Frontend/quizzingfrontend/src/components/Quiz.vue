  ...
  <div>
    ...
    <div v-for="question in questions" v-bind:key="question.questionsId">
      ...
      <ul>
        <li>
          <button
            @click="clickAnswer(question[0].a1, question[0].correctAnswer)"
          >
            {{ question[0].a1 }}
          </button>
        </li>
        <li>
          <button
            @click="clickAnswer(question[0].a2, question[0].correctAnswer)"
          >
            {{ question[0].a2 }}
          </button>
        </li>
        <li>
          <button
            @click="clickAnswer(question[0].a3, question[0].correctAnswer)"
          >
            {{ question[0].a3 }}
          </button>
        </li>
        <li>
          <button
            @click="
              clickAnswer(question[0].correctAnswer, question[0].correctAnswer)
            "
          >
    ...
    <button v-if="questionAnswered" type="submit" @click="nextQuestion()">
      Next question
    </button>
    <p>{{ studentAnswers }}</p>
    <p>{{ correctAnswers }}</p>
...
export default {
  ...
  methods: {
    numberOfQuestions() {
      fetch(
        "http://localhost:3000/api/quiz/numberofquestions/" + this.quizIndex
      )
        .then((res) => res.json())
